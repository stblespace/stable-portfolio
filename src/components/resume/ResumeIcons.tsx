...
const iconClass = "w-6 h-6 xxl:w-7! xxl:h-7! text-[#D7263D] shrink-0"

export function SkillIcon({ icon }: { icon: SkillGroup["icon"] }) {
  switch (icon) {
    case "backend":
      return (
...
export function EducationIcon({ icon }: { icon: "grad" | "bulb" }) {
  if (icon === "grad") {
    return (
      <svg className={iconClass} fill="none" stroke="currentColor" viewBox="0 0 24 24" aria-hidden>
